# dgbook-visual-signature: outdoor-sector-obstruction-map
# dgbook-visual-motif: sector-coverage
# dgbook-learning-focus: read outdoor site direction through roads, buildings, azimuth, and obstruction evidence
# dgbook-visual-primitives: road-grid, sector-fan, building-blocks, obstruction-rays
# dgbook-knowledge-parameters: P02-ku-sector-obstruction; 室外站点勘察要把站向、道路、楼宇遮挡转成覆盖模型; 方位角/下倾角/遮挡距离
from manim import *
import numpy as np


class P02P02OutdoorSiteSurvey(Scene):
    ...
    def outdoorSurveySectorIntro(self):
        title_chip = RoundedRectangle(width=2.2, height=0.52, corner_radius=0.14, color="#0f766e", fill_color="#0f766e", fill_opacity=0.12).move_to(LEFT * 3.55 + DOWN * 0.2)
        title_text = self.label("室外扇区", 18, "#0f766e").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["道路", "楼宇", "站向", "遮挡"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#0f766e", "#2563eb", "#ca8a04", "#dc2626"][index], fill_color=["#0f766e", "#2563eb", "#ca8a04", "#dc2626"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#0f766e", "#2563eb", "#ca8a04", "#dc2626"][index], fill_color=["#0f766e", "#2563eb", "#ca8a04", "#dc2626"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#0f766e", "#2563eb", "#ca8a04", "#dc2626"][index]).move_to(badge.get_center() + DOWN * 0.07)
        ...
        focus = SurroundingRectangle(nodes, color="#0f766e", buff=0.16, corner_radius=0.16)
        ...
        self.play(Indicate(nodes[1], color="#2563eb"), run_time=0.55)
        self.play(FadeOut(page, shift=DOWN * 0.18), run_time=0.45)


    def construct(self):
        self.camera.background_color = "#f8fafc"
        title = self.label("室外站点勘察：把环境建成覆盖模型", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("空间关系、遮挡变化、证据底图", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.outdoorSurveySectorIntro()

        visual_signature = "outdoor-sector-obstruction-map"
        visual_primitives = ["road-grid", "sector-fan", "building-blocks", "obstruction-rays"]
