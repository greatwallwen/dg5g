# dgbook-visual-signature: fault-triage-cause-funnel
# dgbook-visual-motif: triage-funnel
# dgbook-learning-focus: split a test anomaly into coverage, parameter, service, and verification causes
# dgbook-visual-primitives: anomaly-spark, cause-funnel, branch-arrows, verification-stamp
# dgbook-knowledge-parameters: P05-ku-test-anomaly-triage; 测试异常先界定现场现象，再分流到工具、网络或业务侧; 掉线点/速率突降/设备状态
from manim import *
import numpy as np


class P05P05TestTroubleProcess(Scene):
    ...
    def troubleTriageIntro(self):
        title_chip = RoundedRectangle(width=2.2, height=0.52, corner_radius=0.14, color="#dc2626", fill_color="#dc2626", fill_opacity=0.12).move_to(LEFT * 3.55 + DOWN * 0.2)
        title_text = self.label("异常分流", 18, "#dc2626").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["掉线", "覆盖", "参数", "复核"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#dc2626", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_color=["#dc2626", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#dc2626", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_color=["#dc2626", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#dc2626", "#f59e0b", "#7c3aed", "#0f766e"][index]).move_to(badge.get_center() + DOWN * 0.07)
        ...
        focus = SurroundingRectangle(nodes, color="#dc2626", buff=0.16, corner_radius=0.16)
        ...
        self.play(Indicate(nodes[1], color="#f59e0b"), run_time=0.55)
        self.play(FadeOut(page, shift=DOWN * 0.18), run_time=0.45)


    def construct(self):
        self.camera.background_color = "#f8fafc"
        title = self.label("测试问题处理：从异常点分流原因", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("先定位故障现场，再选择覆盖、参数或业务路径", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.troubleTriageIntro()

        visual_signature = "fault-triage-cause-funnel"
        visual_primitives = ["anomaly-spark", "cause-funnel", "branch-arrows", "verification-stamp"]
