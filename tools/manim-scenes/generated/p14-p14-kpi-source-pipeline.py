# dgbook-visual-signature: multi-source-kpi-pipeline-window
# dgbook-visual-motif: source-pipeline
# dgbook-learning-focus: merge PM, alarm, configuration, and baseline data into one time-windowed KPI口径
# dgbook-visual-primitives: source-buckets, time-window, join-pipeline, normalized-output
# dgbook-knowledge-parameters: P14-ku-kpi-source-pipeline; 全网指标采集要把多源数据按同一时间窗归一; 采集粒度/时间窗/归一化输出
from manim import *
import numpy as np


class P14P14KpiSourcePipeline(Scene):
    ...
    def kpiSourcePipelineIntro(self):
        ...
        title_text = self.label("数据源管线", 18, "#2563eb").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["PM", "DT", "告警", "基线"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#2563eb", "#0f766e", "#e11d48", "#f59e0b"][index], fill_color=["#2563eb", "#0f766e", "#e11d48", "#f59e0b"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#2563eb", "#0f766e", "#e11d48", "#f59e0b"][index], fill_color=["#2563eb", "#0f766e", "#e11d48", "#f59e0b"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#2563eb", "#0f766e", "#e11d48", "#f59e0b"][index]).move_to(badge.get_center() + DOWN * 0.07)
        ...
        self.play(Indicate(nodes[1], color="#0f766e"), run_time=0.55)
        self.play(FadeOut(page, shift=DOWN * 0.18), run_time=0.45)


    def construct(self):
        self.camera.background_color = "#f8fafc"
        title = self.label("全网指标采集：多源数据进入同一口径", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("PM、告警、配置和基线按时间窗汇聚", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.kpiSourcePipelineIntro()

        visual_signature = "multi-source-kpi-pipeline-window"
        visual_primitives = ["source-buckets", "time-window", "join-pipeline", "normalized-output"]
