# dgbook-visual-signature: report-evidence-package-binder
# dgbook-visual-motif: evidence-binder
# dgbook-learning-focus: bind charts, logs, retest evidence, and conclusion into a deliverable report
# dgbook-visual-primitives: chart-sheet, log-sheet, retest-sheet, binder-spine
# dgbook-knowledge-parameters: P13-ku-report-evidence-package; 优化报告要把问题、证据、动作、效果和风险装订成结论; 图表引用/日志引用/复测引用完整率
from manim import *
import numpy as np


class P13P13OptimizationReportLoop(Scene):
    ...
    def reportAssemblyIntro(self):
        ...
        title_text = self.label("报告证据包", 18, "#2563eb").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["图表", "日志", "复测", "结论"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#2563eb", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_color=["#2563eb", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#2563eb", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_color=["#2563eb", "#f59e0b", "#7c3aed", "#0f766e"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#2563eb", "#f59e0b", "#7c3aed", "#0f766e"][index]).move_to(badge.get_center() + DOWN * 0.07)
    ...
    def construct(self):
        ...
        title = self.label("优化报告输出：把证据装订成结论", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("图表、日志、复测结果进入同一交付包", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.reportAssemblyIntro()

        visual_signature = "report-evidence-package-binder"
        visual_primitives = ["chart-sheet", "log-sheet", "retest-sheet", "binder-spine"]
