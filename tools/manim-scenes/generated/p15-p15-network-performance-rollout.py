# dgbook-visual-signature: network-performance-rollout-waves
# dgbook-visual-motif: network-rollout
# dgbook-learning-focus: advance performance improvement from pilot layer to full-network rollout and monitoring
# dgbook-visual-primitives: cell-grid, rollout-waves, monitor-band, review-loop
# dgbook-knowledge-parameters: P15-ku-performance-rollout; 全网性能提升要从试点层到全网波次持续监测; 试点增益/扩展批次/稳态监控
from manim import *
import numpy as np


class P15P15NetworkPerformanceRollout(Scene):
    ...
    def performanceRolloutIntro(self):
        ...
        title_text = self.label("全网推广", 18, "#2563eb").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["瓶颈", "策略", "监控", "复盘"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#2563eb", "#7c3aed", "#0f766e", "#f59e0b"][index], fill_color=["#2563eb", "#7c3aed", "#0f766e", "#f59e0b"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#2563eb", "#7c3aed", "#0f766e", "#f59e0b"][index], fill_color=["#2563eb", "#7c3aed", "#0f766e", "#f59e0b"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#2563eb", "#7c3aed", "#0f766e", "#f59e0b"][index]).move_to(badge.get_center() + DOWN * 0.07)
    ...
    def construct(self):
        ...
        title = self.label("5G 全网性能提升实施", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("从试点到全网，以波次推进并持续监测", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.performanceRolloutIntro()

        visual_signature = "network-performance-rollout-waves"
        visual_primitives = ["cell-grid", "rollout-waves", "monitor-band", "review-loop"]
