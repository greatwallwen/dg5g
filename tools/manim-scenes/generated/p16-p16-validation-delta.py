# dgbook-visual-signature: acceptance-delta-baseline-target
# dgbook-visual-motif: acceptance-delta
# dgbook-learning-focus: validate baseline, target, delta, and abnormal review on the same acceptance scale
# dgbook-visual-primitives: baseline-curve, target-band, delta-gap, acceptance-check
# dgbook-knowledge-parameters: P16-ku-acceptance-delta; 验收要在同一尺度上比较基线、目标和差值; 基线值/目标带/差值缺口
from manim import *
import numpy as np


class P16P16ValidationDelta(Scene):
    ...
    def acceptanceDeltaIntro(self):
        title_chip = RoundedRectangle(width=2.2, height=0.52, corner_radius=0.14, color="#e11d48", fill_color="#e11d48", fill_opacity=0.12).move_to(LEFT * 3.55 + DOWN * 0.2)
        title_text = self.label("验收差值", 18, "#e11d48").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["前值", "目标", "差值", "固化"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#e11d48", "#2563eb", "#0f766e", "#f59e0b"][index], fill_color=["#e11d48", "#2563eb", "#0f766e", "#f59e0b"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#e11d48", "#2563eb", "#0f766e", "#f59e0b"][index], fill_color=["#e11d48", "#2563eb", "#0f766e", "#f59e0b"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#e11d48", "#2563eb", "#0f766e", "#f59e0b"][index]).move_to(badge.get_center() + DOWN * 0.07)
        ...
        focus = SurroundingRectangle(nodes, color="#e11d48", buff=0.16, corner_radius=0.16)
    ...
    def construct(self):
        ...
        title = self.label("效果验收：前后曲线同尺比较", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("基线、目标、异常复查", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.acceptanceDeltaIntro()

        visual_signature = "acceptance-delta-baseline-target"
        visual_primitives = ["baseline-curve", "target-band", "delta-gap", "acceptance-check"]
