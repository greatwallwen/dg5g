# dgbook-visual-signature: field-implementation-wavefront
# dgbook-visual-motif: implementation-wave
# dgbook-learning-focus: roll an optimization plan from pilot to execution, observation, and retest
# dgbook-visual-primitives: pilot-site, wavefront-sites, observation-band, retest-marker
# dgbook-knowledge-parameters: P11-ku-field-implementation; 优化方案实施要从试点、执行、观察到复测分批落地; 试点达标率/变更窗口/复测结果
from manim import *
import numpy as np


class P11P11OptimizationImplementation(Scene):
    ...
    def implementationWaveIntro(self):
        title_chip = RoundedRectangle(width=2.2, height=0.52, corner_radius=0.14, color="#16a34a", fill_color="#16a34a", fill_opacity=0.12).move_to(LEFT * 3.55 + DOWN * 0.2)
        title_text = self.label("实施波次", 18, "#16a34a").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["试点", "执行", "观察", "复测"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#16a34a", "#7c3aed", "#0891b2", "#f59e0b"][index], fill_color=["#16a34a", "#7c3aed", "#0891b2", "#f59e0b"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#16a34a", "#7c3aed", "#0891b2", "#f59e0b"][index], fill_color=["#16a34a", "#7c3aed", "#0891b2", "#f59e0b"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#16a34a", "#7c3aed", "#0891b2", "#f59e0b"][index]).move_to(badge.get_center() + DOWN * 0.07)
        ...
        focus = SurroundingRectangle(nodes, color="#16a34a", buff=0.16, corner_radius=0.16)
        ...
        self.play(Indicate(nodes[1], color="#7c3aed"), run_time=0.55)
        self.play(FadeOut(page, shift=DOWN * 0.18), run_time=0.45)


    def construct(self):
        self.camera.background_color = "#f8fafc"
        title = self.label("方案实施：现场动作如何分批落地", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("从试点、变更、观察到复测，控制执行节奏", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.implementationWaveIntro()

        visual_signature = "field-implementation-wavefront"
        visual_primitives = ["pilot-site", "wavefront-sites", "observation-band", "retest-marker"]
