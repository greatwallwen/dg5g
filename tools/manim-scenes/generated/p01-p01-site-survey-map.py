# dgbook-visual-signature: floorplan-layered-pin-map
# dgbook-visual-motif: layered-floorplan
# dgbook-learning-focus: separate indoor location, device object, photo evidence, and form record layers
# dgbook-visual-primitives: floorplan, route-pins, cabinet-layer, evidence-stack
# dgbook-knowledge-parameters: P01-ku-site-evidence; 室内环境采集要把位置、设备、照片和表单分层; 经纬度/楼层/设备编号完整率
from manim import *
import numpy as np


class P01P01SiteSurveyMap(Scene):
    ...
    def siteSurveyMapIntro(self):
        ...
        title_text = self.label("室内底图", 18, "#0f766e").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["机房", "走线", "机柜", "表单"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#0f766e", "#2563eb", "#f59e0b", "#7c3aed"][index], fill_color=["#0f766e", "#2563eb", "#f59e0b", "#7c3aed"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#0f766e", "#2563eb", "#f59e0b", "#7c3aed"][index], fill_color=["#0f766e", "#2563eb", "#f59e0b", "#7c3aed"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#0f766e", "#2563eb", "#f59e0b", "#7c3aed"][index]).move_to(badge.get_center() + DOWN * 0.07)
    ...
    def construct(self):
        ...
        title = self.label("室内环境信息采集", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("空间定位、现场证据和底图分层", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.siteSurveyMapIntro()

        visual_signature = "floorplan-layered-pin-map"
        visual_primitives = ["floorplan", "route-pins", "cabinet-layer", "evidence-stack"]
