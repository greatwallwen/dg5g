# dgbook-visual-signature: threshold-drilldown-low-valley
# dgbook-visual-motif: threshold-drilldown
# dgbook-learning-focus: map KPI low valleys to threshold, alert, and field segment evidence
# dgbook-visual-primitives: metric-axis, threshold-line, low-valley-lens, segment-card
# dgbook-knowledge-parameters: P06-ku-kpi-low-valley; 测试数据诊断要用多 KPI 解释低谷片段; RSRP/SINR/PRB/事件点
from manim import *
import numpy as np


class P06P06TestDataKpiDiagnosis(Scene):
    ...
    def kpiThresholdIntro(self):
        ...
        title_text = self.label("阈值诊断", 18, "#2563eb").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["RSRP", "SINR", "事件", "小区"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#2563eb", "#0f766e", "#dc2626", "#f59e0b"][index], fill_color=["#2563eb", "#0f766e", "#dc2626", "#f59e0b"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#2563eb", "#0f766e", "#dc2626", "#f59e0b"][index], fill_color=["#2563eb", "#0f766e", "#dc2626", "#f59e0b"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#2563eb", "#0f766e", "#dc2626", "#f59e0b"][index]).move_to(badge.get_center() + DOWN * 0.07)
    ...
    def construct(self):
        ...
        title = self.label("测试数据分析：阈值下探定位低谷", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("把曲线低谷、门限线和现场片段对应起来", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.kpiThresholdIntro()

        visual_signature = "threshold-drilldown-low-valley"
        visual_primitives = ["metric-axis", "threshold-line", "low-valley-lens", "segment-card"]
