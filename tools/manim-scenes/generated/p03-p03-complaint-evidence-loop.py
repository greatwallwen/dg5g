# dgbook-visual-signature: complaint-evidence-chain-loop
# dgbook-visual-motif: closed-evidence-loop
# dgbook-learning-focus: connect complaint, location, logs, and retest result into one causal chain
# dgbook-visual-primitives: voice-ticket, geo-pin, log-strip, closed-loop-arrows
# dgbook-knowledge-parameters: P03-ku-complaint-chain; 投诉处理要把用户主诉和网络侧证据闭环; 投诉时间窗/小区/业务类型匹配度
from manim import *
import numpy as np


class P03P03ComplaintEvidenceLoop(Scene):
    ...
    def complaintEvidenceChainIntro(self):
        ...
        title_text = self.label("投诉证据链", 18, "#2563eb").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["主诉", "位置", "日志", "复测"]):
            x = -1.35 + index * 1.18
            badge = RoundedRectangle(width=0.94, height=0.58, corner_radius=0.13, color=["#2563eb", "#f59e0b", "#dc2626", "#0f766e"][index], fill_color=["#2563eb", "#f59e0b", "#dc2626", "#0f766e"][index], fill_opacity=0.13).move_to(RIGHT * x + DOWN * 0.2)
            marker = Circle(radius=0.12, color=["#2563eb", "#f59e0b", "#dc2626", "#0f766e"][index], fill_color=["#2563eb", "#f59e0b", "#dc2626", "#0f766e"][index], fill_opacity=0.82).move_to(badge.get_top() + DOWN * 0.14)
            label = self.label(name, 13, ["#2563eb", "#f59e0b", "#dc2626", "#0f766e"][index]).move_to(badge.get_center() + DOWN * 0.07)
    ...
    def construct(self):
        ...
        title = self.label("投诉信息：从用户感知追到证据链", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("把主诉、位置、日志和复测结论串成闭环", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        self.complaintEvidenceChainIntro()

        visual_signature = "complaint-evidence-chain-loop"
        visual_primitives = ["voice-ticket", "geo-pin", "log-strip", "closed-loop-arrows"]
