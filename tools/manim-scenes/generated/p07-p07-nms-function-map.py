# dgbook-visual-signature: nms-capability-hub-spokes
# dgbook-visual-motif: capability-hub
# dgbook-learning-focus: organize NMS support around alarms, configuration, performance, topology, and tickets
# dgbook-visual-primitives: center-hub, spoke-modules, event-badges, workorder-rail
# dgbook-knowledge-parameters: P07-ku-nms-capability; 网管功能按告警、配置、性能、拓扑和工单分工; 告警时间窗/性能粒度/配置版本
from manim import *
import numpy as np


class P07P07NmsFunctionMap(Scene):
    ...
    def nmsHubIntro(self):
        ...
        title_text = self.label("网管能力", 18, "#0f766e").move_to(title_chip)
        nodes = VGroup()
        for index, name in enumerate(["告警", "性能", "参数", "工单"]):
            x = -1.35 + index * 1.18
    ...
    def construct(self):
        ...
        title = self.label("网管功能图：从中心能力看支撑对象", 32, "#0f172a").to_edge(UP, buff=0.32)
        subtitle = self.label("围绕告警、配置、性能、拓扑和工单展开", 18, "#0f766e").next_to(title, DOWN, buff=0.1)
        ...
        visual_signature = "nms-capability-hub-spokes"
        visual_primitives = ["center-hub", "spoke-modules", "event-badges", "workorder-rail"]
