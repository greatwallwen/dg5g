...
P01_ACTIVITY_SPECS: dict[str, dict[str, Any]] = {
    ...
    "P1T1-N02-application-01": {
        ...
        "materials": [
            ...
            {"id": "aau01-port", "label": "AAU-01 OPT-1", "detail": "线缆标签 FO-22。"},
            {"id": "aau02-port", "label": "AAU-02 OPT-1", "detail": "线缆标签 FO-29，与 ODF-B/04 出端的 FO-22 不一致。"},
        ],
        "interaction": {
            ...
            "candidates": [
                {
                    "id": "candidate-a",
                    "label": "候选链路 A",
                    "materialIds": ["bbu-port", "odf-in", "odf-out", "aau01-port"],
                },
                {
                    "id": "candidate-b",
                    "label": "候选链路 B",
                    "materialIds": ["bbu-port", "odf-in", "odf-out", "aau02-port"],
                },
            ],
            "exclusionReasons": [
                {"id": "far-end-label-mismatch", "label": "排除 B：ODF 出端 FO-22 与 AAU-02 端 FO-29 不一致"},
                {"id": "physical-distance", "label": "排除 B：AAU-02 与机柜距离更远"},
                {"id": "indicator-state", "label": "排除 B：AAU-02 指示灯状态不同"},
            ],
        },
        "targetedFeedback": {
            "passed": "候选 A 的 FO-17—JP-08—FO-22 编号连续，并已用 FO-29 断点排除候选 B。",
            "failed": "候选链路或排除依据不成立；必须逐段核对本端、ODF 跳接和对端线缆编号，不能用距离或指示灯猜测。",
        },
    ...
    "P1T1-N02-transfer-01": {
        ...
        "materials": [
            {"id": "aau-power-port", "label": "AAU 设备侧证据", "detail": "HY-01 A 座楼顶 S1 扇区，AAU-01 铭牌与电源端口 PWR-1 同框；线缆标签为 PWR-DC-17。"},
            {"id": "power-cable-trace", "label": "直流电源线连续走线", "detail": "PWR-DC-17 从 AAU-01 PWR-1 连续追踪至 01 号机房 K03 机柜。"},
            {"id": "dcdu-terminal", "label": "配电侧证据", "detail": "K03 内 DCDU-01 的 -48V/12 端子挂接 PWR-DC-17，端子标签和线缆标签清晰同框。"},
        ],
        "interaction": {
            "type": "record-form",
            "fields": [
                {"id": "aauIdentity", "label": "AAU 身份", "placeholder": "例如 AAU-01"},
                {"id": "aauPowerPort", "label": "AAU 电源端口", "placeholder": "例如 PWR-1"},
                {"id": "powerCableLabel", "label": "直流电源线标签", "placeholder": "例如 PWR-DC-17"},
                {"id": "distributionDevice", "label": "配电设备", "placeholder": "例如 DCDU-01"},
                {"id": "distributionTerminal", "label": "配电端子", "placeholder": "例如 -48V/12"},
                {"id": "powerDirection", "label": "供电方向", "placeholder": "例如 DCDU-01 -48V/12 → AAU-01 PWR-1"},
            ],
        },
        "targetedFeedback": {
            "passed": "AAU 身份、设备侧端口、同号电源线和配电端子已闭合为可回查的直流供电证据链。",
            "failed": "直流供电记录仍与证据包不一致；请逐项核对 AAU 身份、PWR-1、PWR-DC-17 和 DCDU-01 -48V/12。",
        },
        "transferTarget": "形成可直接汇入 P01 专业成果的 AAU 直流供电证据记录。",
    },
    ...
    "P1T1-N02": {
        ...
        "practices": {
            ...
            "application": [_practice(
                "P1T1-N02-application-01",
                "对比两条候选链路，选择唯一能够闭合的链路，并用明确的标签断点排除另一条。",
                ["候选 A 与候选 B 均完整呈现", "本端、中间跳接和对端编号连续", "排除理由引用具体标签断点"],
                "结论必须同时引用起点、中间路径、终点和被排除候选的断点，不能用距离或指示灯猜测。",
                ["逐段标出 FO-17、JP-08、FO-22 和 FO-29", "先找出编号连续的候选链路", "再用 FO-22 与 FO-29 的断点排除另一条后重试"],
            )],
            "transfer": [
                _practice(
                    "P1T1-N02-transfer-01",
                    "将证据方法迁移到直流供电链：依据三组现场材料，形成 AAU-01 由哪一个 DCDU 端子供电的结构化记录。",
                    ["AAU 身份与 PWR-1 设备侧端口", "PWR-DC-17 两端同号", "DCDU-01 -48V/12 配电端子与供电方向"],
                    "设备类型改变后，仍须用唯一身份、双端同号和明确方向闭合证据链。",
                    ["先确定 AAU-01 与 PWR-1", "沿 PWR-DC-17 连续追踪", "在 DCDU-01 -48V/12 端子闭合并写明供电方向"],
                ),
                _practice(
                    "P1T1-N02-remediation-revision-01",
