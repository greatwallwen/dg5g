...
test('a v8 production-shaped demo upgrades through v13 without deleting an unknown legacy runtime fact', () => {
  const fixture = createTestDatabase();
  try {
    ...
    assert.deepEqual(migrateDatabase(fixture.database).appliedVersions, [9, 10, 11, 12, 13]);
    seedDemo(fixture.database);
