...
test('renders the P1 project chain with stable audit selectors and no locked task links', () => {
  ...
  try {
    ...
    assert.match(html, /data-primary-action="true"[^>]*data-p1-next-action="P1T1-N02"|data-p1-next-action="P1T1-N02"[^>]*data-primary-action="true"/);
    assert.match(html, /data-p1-task="P01"/);
    assert.match(html, /data-p1-task="P02"/);
    assert.match(html, /data-p1-task="P03"/);
    assert.match(html, /data-p1-node-state="micro-practice-passed"/);
    assert.match(html, /data-p1-node-state="locked"/);
    assert.match(html, /data-p1-next-action="P1T1-N02"/);
    assert.match(html, /data-p1-portfolio-status="collecting"/);
    assert.match(html, /data-p1-portfolio-link="collecting"/);
