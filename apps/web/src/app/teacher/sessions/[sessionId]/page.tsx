...
export default async function TeacherSessionPage({ params }: { params: { sessionId: string } }) {
  ...
  try {
    await getTeacherSession(params.sessionId);
  } catch (error) {
  ...
  return <TeacherConsoleClient displayName={actor.displayName} initialSnapshot={snapshot} profiles={loadDemoTaskProfiles()} />;
}
